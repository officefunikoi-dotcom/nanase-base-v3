...
const baseStyle = `
  ...
  * { box-sizing: border-box; margin: 0; padding: 0; font-family: -apple-system, sans-serif; }
  ...
  .my-stamp-box { background: #fff9e6; border: 2px solid #ffcc00; padding: 12px; border-radius: 12px; margin-bottom: 12px; }
  ...
  .stamp-list-scroll { max-height: 400px; overflow-y: auto; }
  .stamp-row { display: flex; justify-content: space-between; align-items: center; padding: 12px 5px; border-bottom: 1px solid #f0f0f0; }
  .rank-badge { margin-right: 8px; font-size: 1.1rem; }
  .count-badge { background: var(--sub); color: var(--main); padding: 2px 10px; border-radius: 20px; font-weight: 800; }
...
async function mainPortalHtml(data) {
  ...
  const sections = {
    ...
    stamps: `<div class="card"><h3>Listener Ranking</h3>
      <div id="my-stamp-section" class="my-stamp-box">
        <p style="font-size:0.7rem; color:#d4a017; font-weight:bold; margin-bottom:5px;">✨ マイスタンプ表示</p>
        <div style="display:flex; gap:5px;">
          <input type="text" id="my-name-input" class="search-input" style="margin:0; flex-grow:1;" placeholder="自分の名前を保存">
          <button class="btn btn-sm" onclick="saveMyName()">保存</button>
        </div>
        <div id="my-count-display" style="display:none; justify-content:space-between; margin-top:10px; font-weight:bold; color:var(--main); font-size:1.1rem; border-top:1px dashed #ffcc00; padding-top:10px;">
          <span id="display-name"></span><span id="display-count" class="count-badge"></span>
        </div>
      </div>
      <input type="text" class="search-input" placeholder="名前を検索..." oninput="filterUserStamps(this.value)">
      <div class="stamp-list-scroll" id="user-stamp-list">${sortedStamps.map((s, index) => {
        ...
        return `<div class="stamp-row user-item" data-name="${s.name}">
          <span><span class="rank-badge">${rank}</span>${s.name}</span>
...
async function adminDashboardHtml(data) {
  ...
  return `<!DOCTYPE html><html lang="ja"><head><meta charset="UTF-8"><meta name="viewport" content="width=device-width,initial-scale=1.0"><style>${baseStyle}</style></head>
    ...
    <div class="card">
      <h3 style="color:var(--main); font-weight:bold; margin-bottom:10px;">👤 スタンプ管理（数が多い順）</h3>
      <input type="text" class="search-input" placeholder="リスナーを検索..." oninput="filterAdminStamps(this.value)">
      <div id="stamp-list" style="max-height:450px; overflow-y:auto;">${sortedAdminStamps.map(s => `
        <div class="item-row admin-stamp-item">
          <input value="${s.name}" class="s-name" style="flex-grow:1; font-size:0.85rem;">
          <button class="btn-sm" style="background:var(--main);color:white" onclick="this.nextElementSibling.value++">＋</button>
          <input type="number" value="${s.count}" class="s-count" style="width:40px;text-align:center;">
          <button class="btn-sm btn-outline" onclick="this.previousElementSibling.value--">－</button>
          <div style="display:flex; flex-direction:column; gap:2px;">
            <button class="btn-sm btn-outline" style="padding:0 4px; font-size:10px;" onclick="moveUp(this.parentElement.parentElement)">▲</button>
            <button class="btn-sm btn-outline" style="padding:0 4px; font-size:10px;" onclick="moveDown(this.parentElement.parentElement)">▼</button>
          </div>
  ...
  <script>
    ...
    function addStamp(){ const d=document.createElement('div'); d.className='item-row admin-stamp-item'; d.innerHTML='<input class="s-name" style="flex-grow:1"><button class="btn-sm" style="background:var(--main);color:white" onclick="this.nextElementSibling.value++">＋</button><input type="number" value="0" class="s-count" style="width:40px;text-align:center;"><button class="btn-sm btn-outline" onclick="this.previousElementSibling.value--">－</button><div style="display:flex; flex-direction:column; gap:2px;"><button class="btn-sm btn-outline" style="padding:0 4px; font-size:10px;" onclick="moveUp(this.parentElement.parentElement)">▲</button><button class="btn-sm btn-outline" style="padding:0 4px; font-size:10px;" onclick="moveDown(this.parentElement.parentElement)">▼</button></div><button class="btn-sm btn-red" onclick="this.parentElement.remove()">×</button>'; document.getElementById('stamp-list').prepend(d); }
    async function save(){
